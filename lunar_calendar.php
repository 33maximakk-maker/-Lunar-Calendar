...
define('CONFIG_FILE', 'lunar_config.json');
...
function sunPosition($jd) {
    ...
    $lon = fmod(280.4665 + 36000.7698 * $T + $C, 360);
    return $lon * M_PI / 180;
}
...
function asciiPhase($illumination) {
...
$opts = getopt("", ["date:", "lat:", "lon:", "phase-only", "all", "save-location:"]);
...
$lat = $opts['lat'] ?? $config['saved_location']['lat'] ?? DEFAULT_LAT;
$lon = $opts['lon'] ?? $config['saved_location']['lon'] ?? DEFAULT_LON;
...
echo "\n🌓 Phase: {$result['phase']} (" . number_format($result['illumination'], 1) . "% illuminated)\n";
echo "📅 Moon age: " . number_format($result['age'], 1) . " days\n";
