...
OptionParser.new do |opts|
  opts.banner = "Usage: lunar_calendar.rb [options]"
  ...
  opts.on("--all", "Show all details") { options[:all] = true }
  opts.on("--save-location NAME", "Save location name") { |v| options[:save_location] = v }
end.parse!

# Load config
config = { "saved_location" => { "lat" => DEFAULT_LAT, "lon" => DEFAULT_LON } }
if File.exist?(CONFIG_FILE)
  config = JSON.parse(File.read(CONFIG_FILE)) rescue config
end

if options[:save_location]
  config["saved_location"] = { "name" => options[:save_location], "lat" => options[:lat] || DEFAULT_LAT, "lon" => options[:lon] || DEFAULT_LON }
  File.write(CONFIG_FILE, JSON.pretty_generate(config))
  puts "✅ Location '#{options[:save_location]}' saved."
end

lat = options[:lat] || config["saved_location"]["lat"] || DEFAULT_LAT
lon = options[:lon] || config["saved_location"]["lon"] || DEFAULT_LON
...
puts "\n🌓 Phase: #{result[:phase]} (#{result[:illumination].round(1)}% illuminated)"
puts "📅 Moon age: #{result[:age].round(1)} days"
puts "♑ Zodiac: #{result[:zodiac]}"
puts "\nVisual:"
puts ascii_phase(result[:illumination])
