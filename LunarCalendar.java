...
class Config {
    public SavedLocation saved_location = new SavedLocation();
}
...
public class LunarCalendar {
    ...
    private static final String CONFIG_FILE = "lunar_config.json";
    ...
    public static LunarResult moonPhase(LocalDateTime dt) {
        ...
        LunarResult r = new LunarResult();
        r.phase = phase;
        r.illumination = illumination * 100;
        r.age = age;
        r.zodiac = zodiac;
        return r;
    }

    public static String asciiPhase(double illumination) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (params.containsKey("save-location")) {
            config.saved_location.name = params.get("save-location");
            config.saved_location.lat = Double.parseDouble(params.getOrDefault("lat", String.valueOf(DEFAULT_LAT)));
            config.saved_location.lon = Double.parseDouble(params.getOrDefault("lon", String.valueOf(DEFAULT_LON)));
            Files.write(Paths.get(CONFIG_FILE), gson.toJson(config).getBytes());
            System.out.println("✅ Location '" + config.saved_location.name + "' saved.");
        }

        double lat = params.containsKey("lat") ? Double.parseDouble(params.get("lat")) : config.saved_location.lat;
        double lon = params.containsKey("lon") ? Double.parseDouble(params.get("lon")) : config.saved_location.lon;
        ...
        if (params.containsKey("phase-only")) {
        ...
        System.out.printf("\n🌓 Phase: %s (%.1f%% illuminated)\n", result.phase, result.illumination);
