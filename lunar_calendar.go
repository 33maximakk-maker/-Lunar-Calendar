...
func sunPosition(jd float64) float64 {
	...
	lon := 280.4665 + 36000.7698*T + C
	return math.Mod(lon, 360) * math.Pi / 180
}

func moonPhase(t time.Time) LunarResult {
	jd := julianDay(t)
	...
	return LunarResult{
		...
		Age:          age,
		Elongation:   elong * 180 / math.Pi,
		Zodiac:       zodiac,
	}
}

func asciiPhase(illumination float64) string {
	if illumination < 1 {
		return "🌑 (New Moon)"
	}
	if illumination < 20 {
...
func main() {
	var (
		...
		all          = flag.Bool("all", false, "Show all details")
		saveLocation = flag.String("save-location", "", "Save location name")
	)
	flag.Parse()

	// Load config
	configFile := "lunar_config.json"
	var config Config
