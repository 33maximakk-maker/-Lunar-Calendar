...
const CONFIG_FILE = 'lunar_config.json';
...
function sunPosition(jd) {
    ...
    let lon = (280.4665 + 36000.7698 * T + C) % 360;
...
program
    ...
    .option('--save-location <name>', 'Save location name')
    .parse(process.argv);

const opts = program.opts();

// Load config
let config = { saved_location: { lat: DEFAULT_LAT, lon: DEFAULT_LON } };
...
if (opts.saveLocation) {
    config.saved_location = { name: opts.saveLocation, lat: opts.lat, lon: opts.lon };
...
console.log(`\n🌓 Phase: ${result.phase} (${result.illumination.toFixed(1)}% illuminated)`);
console.log(`📅 Moon age: ${result.age.toFixed(1)} days`);
console.log(`♑ Zodiac: ${result.zodiac}`);
